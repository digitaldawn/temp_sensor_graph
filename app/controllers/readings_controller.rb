class ReadingsController < ApplicationController
  def index
    ...
    @readings = Reading.where(created_at: (Time.now - 1.day)..Time.now).map { |reading| { x: (reading.created_at.strftime(“%M”).to_i - Time.now.strftime(“%M”).to_i) , y: reading.temperature } }
